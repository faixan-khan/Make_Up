...
def landmark_detection(image):
    ...
    cv2.imshow("Maskked_image", output_marked_image)
    cv2.waitKey(0)
...
sourceImagePath = "./MakeUp/ref.jpg"
sourceImage = cv2.imread(sourceImagePath)
r = image_height * 1.0 / sourceImage.shape[0]
dim = (int(sourceImage.shape[1] * r), image_height)
output_image1, feature_points1, triangulation1, line = landmark_detection(sourceImage)
sourceImage = cv2.resize(sourceImage, dim, interpolation=cv2.INTER_AREA)
sourceTuple = (output_image1, feature_points1, triangulation1,line)

MASK_dash = (line - np.min(line))/(np.max(line)-np.min(line))
cv2.imshow("Mask_Generated",MASK_dash)